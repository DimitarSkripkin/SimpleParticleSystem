...
void BatchRecorder::FinishRecording() {
    for (auto & ptr : subBatches) {
        ...
        if (drawBatch->vao.IsStarted()) {
            drawBatch->vertexBuffer.BindData(drawBatch->vertices.data(), sizeof(SpriteVertex) * static_cast<uint32_t>(drawBatch->vertices.size()), GL_DYNAMIC_DRAW);
...
void BatchRecorder::Add(const DrawQuad & drawQuad, const glm::mat4 & mat) {
    std::map<Texture *, DrawBatch2D *>::iterator batch;
